            ...
            let res = match state {
                State::Start(update) => match update {
                    DisplayUpdate::Bitmap(bitmap) => {
                        ...
                        encoder.bitmap_update_framebuffer(bitmap.clone(), &diffs);
                        self.state = State::BitmapDiffs { diffs, bitmap };
                        continue;
                    }
                ...
                State::BitmapDiffs { mut diffs, bitmap } => {
                    let rect = diffs.pop()?;
